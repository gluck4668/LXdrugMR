...
mr_data_processing <- function(tart_gene,chr_exp,pos_exp_start,pos_exp_end,
                               exp_dat,clump_kb,clump_r2,exposure_name,
                               exp_p,beta_exp,se_exp,effect_allele_exp,
                               other_allele_exp,eaf_exp,pval_exp,
                              outcome_name,out_dat,beta_out,se_out,
                          effect_allele_out,other_allele_out,eaf_out,pval_out){

#------- Exposure data -------------------------------------------------------
#------ gwas id format------
if(!grepl("[.]",exp_dat,ignore.case = T)){
 print("Begin to read the exposure data, it may take a long time......")
 exp_set <- exp_gwas_id (tart_gene,chr_exp,pos_exp_start,pos_exp_end,
...
#------VCF.gz format------
 if(grepl("vcf.gz",exp_dat,ignore.case = T)){
  print("Begin to read the exposure data, it may take a long time......")
  exp_set <- exp_vcf_data(tart_gene,chr_exp,pos_exp_start,pos_exp_end,exp_dat,exp_p)
  exp_data <- exp_set$exp_data
  exp_snp <- exp_set$exp_snp
...
if(grepl(".bgz",exp_dat,ignore.case = T)){
    ...
    exp_set <- exp_bgz_data (tart_gene,chr_exp,pos_exp_start,pos_exp_end,
...
{
  print("Begin to read the exposure data, it may take a long time......")
    exp_set <- exp_other_format(tart_gene,chr_exp,pos_exp_start,pos_exp_end,
                                exp_dat,snp_exp,pval_exp,beta_exp,
                             se_exp,effect_allele_exp,other_allele_exp,
                             eaf_exp,clump_kb,clump_r2)
